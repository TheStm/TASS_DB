"""Helpers to build a GDS projection and fetch the shortest route between airports."""
...
GRAPH_NAME = os.getenv("GDS_GRAPH_NAME", "airportDistance")
...
def _connect_with_retry():
    ...
    while True:
        try:
            ...
            return driver
        except ServiceUnavailable as exc:  # Neo4j may still be starting
            attempts += 1
...
class ShortestRoute:
    ...
    def to_dict(self) -> Dict[str, object]:
        return {
            ...
            "totalDistanceNm": self.total_distance_nm,
        }
...
def shortest_route(
    ...
    *,
    ...
    driver=None,
) -> Optional[ShortestRoute]:
    """Return shortest route and total distance (nm) between two airport codes.

    Args:
        source_code: Airport code for route start.
        target_code: Airport code for route end.
        graph_name: Name of the GDS graph projection to use.
        rebuild_graph: If True, drop and rebuild the projection before query.
        driver: Optional existing Neo4j driver; if absent a new one is created.

    Returns:
        ShortestRoute dataclass with stops and total_distance_nm, or None when no path exists.
    """
    ...
    try:
        with neo4j_driver.session() as session:
            ...
            return ShortestRoute(stops=stops, total_distance_nm=record["totalDistanceNm"])
    finally:
        if created_driver:
            neo4j_driver.close()
...
if __name__ == "__main__":
    # Example manual run: python smoska/shortest_path_distance.py EPWA LPPT
    import sys

    if len(sys.argv) != 3:
        print("Usage: python smoska/shortest_path_distance.py <SRC_CODE> <DST_CODE>")
        raise SystemExit(1)

    result = shortest_route(sys.argv[1], sys.argv[2])
    if result is None:
